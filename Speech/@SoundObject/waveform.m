...
files = get(o,'Names');
if ischar(index)
    index = find(strcmpi(files,index));
end
...
if strcmp(NoiseType,'SpectSmooth'),
    [w,fs] = audioread([soundpath filesep 'ds1_' files{index}]);
else
    if strfind(files{index},'.wav')
        [w,fs] = audioread([soundpath filesep files{index}]);
    else
        [w,fs] = audioread([soundpath filesep files{index},'.wav']);
    end
end
...
% possible:
if isfield(get(o),'Duration') && get(o,'Duration')>0
...
if isfield(get(o),'SNR')
    ...
    if get(o,'SNR') <100 && ~strcmpi(NoiseType,'none') && ~strcmpi(NoiseType,'SpectSmooth')
        if strcmpi(NoiseType,'white')
            ...
            [n,fs] = audioread([soundpath filesep lower(NoiseType) '.wav']);
...
if isfield(get(o),'ReverbTime') && get(o,'ReverbTime')>0
    len = length(w);
    randn('seed',index);
