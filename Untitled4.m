...
for k=1:n2-t
y2=d(k+n2-1:-1:k);          % last item of is the most recent
...
for k=n2-t:N-n2-2
% going left to right
y1=d(k-n2+1:k);			    	% autocorr require last item the recent
...
y=d(k:-1:k-n+1);                % fold d (input y) for convolution
...
aa(3) = d(k);                   % noisy speech from original file
aa(4) = Rxy1.'*Ry1;
aa(5) = Rxy2.'*Ry2;
if k == 5171
    i=i;
...
%        bp  w
alpha = [15 13];  %delay of the filters
Y = delay(Y,alpha(2));
d = delay(d,alpha(1));
% sY = delay(sY,alpha(2));
% sd = delay(sd,alpha(1));
